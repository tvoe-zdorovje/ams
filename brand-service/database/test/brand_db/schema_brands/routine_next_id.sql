...
DECLARE
    seq_name REGCLASS := 'brands.brand_id_seq';
    actual BIGINT;
    expected BIGINT;
BEGIN
    expected := format('%s3221100', nextval(seq_name) + 100);

    actual := brands.next_id(seq_name);

    RETURN NEXT is(
        actual,
        expected,
        'Routine brands.next_id must return correct result'
    );
END;
$$ LANGUAGE plpgsql;
