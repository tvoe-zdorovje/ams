...
CREATE OR REPLACE FUNCTION tests.test_update_studio_routine_structure() RETURNS SETOF TEXT AS $$
declare
    schema_name TEXT := 'studios';
    routine_name TEXT := 'update_studio';
BEGIN
...
select plan(1);
select has_function('test_update_studio_routine_structure');
select finish(true);
