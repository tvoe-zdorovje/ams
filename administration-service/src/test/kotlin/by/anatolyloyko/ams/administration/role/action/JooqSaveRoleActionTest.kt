...
class JooqSaveRoleActionTest : WithAssertions {
    ...
    fun `must call save role without permissions routine and return ID`() = mockkStatic(ROUTINES_REFERENCE) {
        every { saveRole(any(), any(), any(), any(), any(), any()) } returns ROLE_ID

        val result = action(STUDIO_ID, NEW_ROLE)

        assertThat(result).isEqualTo(ROLE_ID)
        verify(exactly = 1) {
            saveRole(any(), STUDIO_ID, NEW_ROLE.id, NEW_ROLE.name, NEW_ROLE.description, emptyArray())
        }
    }
    ...
    fun `must call save role with permissions routine and return ID`() = mockkStatic(ROUTINES_REFERENCE) {
        ...
        val result = action(STUDIO_ID, NEW_ROLE, permissions)

        assertThat(result).isEqualTo(ROLE_ID)
        verify(exactly = 1) {
            saveRole(any(), STUDIO_ID, NEW_ROLE.id, NEW_ROLE.name, NEW_ROLE.description, permissions.toTypedArray())
        }
    }

    @Test
    fun `must throw an exception when routine invocation result is null`() = mockkStatic(ROUTINES_REFERENCE) {
        every { saveRole(any(), any(), any(), any(), any(), any()) } returns null

        assertThatThrownBy { action(STUDIO_ID, NEW_ROLE) }
