...
CREATE OR REPLACE FUNCTION tests.test_create_studio_role_routine_structure() RETURNS SETOF TEXT AS $$
DECLARE
    schema_name TEXT := 'administration';
    routine_name TEXT := 'create_studio_role';
BEGIN
...
CREATE OR REPLACE FUNCTION tests.test_create_studio_role_routine_behavior() RETURNS SETOF TEXT AS $$
DECLARE
    routine_name TEXT := 'administration.create_studio_role';
...
BEGIN
    ...
    SELECT administration.create_studio_role(
        studio_id,
        role_name,
        role_description,
        permissions
...
select plan(2);
select has_function('test_create_studio_role_routine_structure');
select has_function('test_create_studio_role_routine_behavior');
select finish(true);
